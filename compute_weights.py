...
def normalize_loss_weights(
    ...
    # Initialize loss trackers (add bandwise losses)
    loss_values = {
    ...
    if yaml_filepath and os.path.exists(yaml_filepath):
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(train_loader):
                ...
                real_band_signals, _ = split_signal_frequency_bands(real_music, fs=sample_rate)
                ...
                rythm_and_beats_cost = rythm_and_beats_loss(
                    real_music, fake_music_g,
                    sample_rate=sample_rate, rhythm_weight=rhythm_weight, pitch_weight=pitch_weight, btw_weight=btw_weight
                )
                diversity_cost = -torch.mean(torch.var(fake_music_g, dim=0))
                silence_cost = silence_loss(fake_music_g, beats)
                alternation_cost = alternation_loss(beats, fake_music_g)
                beat_duration_cost = beat_duration_loss(fake_music_g, sample_rate, min_duration=beat_min_duration, max_duration=beat_max_duration)
                ...
                cp_cost = combined_perceptual_loss(
                    fake_music_g, real_music, sample_rate, scales=[512, 1024, 2048],
                    mel_weight=comb_percept_lambda,
                    complex_weight=comb_percept_lambda,
                    multi_scale_weight=comb_percept_lambda
                )
                rhythm_cost = rhythm_enforcement_loss(
                    fake_music_g, beats,
                ...
                # Bandwise generator/discriminator loss (returns scalar and dict)
                band_g_loss, band_g_terms = bandwise_generator_loss(
                    fake_band_signals, real_band_signals, fband_signals, real_music,
                    criterion_reconstruction, criterion_gan, discriminator, real_labels, loss_weights=None
                )
                band_d_loss, band_d_terms = bandwise_discriminator_loss(
        ...
        loss_weights = {loss_name: 0.10 / max(abs(value), 1e-3) for loss_name, value in average_loss_values.items()}
