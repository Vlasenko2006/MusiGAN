...
def save_sample_as_numpy(generator, device, music_out_folder, epoch, noise_dim, n_channels=2, num_samples=10, prefix=''):
    ...
    with torch.no_grad():
        for i in range(num_samples):
            ...
            noise = noise = noise_fun(batch_size = 1, n_channels = 2, seq_len = noise_dim, device =device) #torch.randn(1, noise_dim).to(device)  # Batch size = 1

            # Generate music
            generated_music, _, _ = generator(noise)
