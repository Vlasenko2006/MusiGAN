...
class VariationalEncoderDecoder(nn.Module):
    def __init__(self, input_dim, num_heads=2,
        ...
        self.gaussians = nn.Sequential(
            nn.Linear(2 * self.song_duration, self.num_of_gaussians * 4),
            nn.ReLU(),
    ...
    def decoder(self, z):
        ...
        if self.use_gaussians:
            reconstructed_flat = reconstructed.view(reconstructed.size(0), -1)  # Flatten the spatial dimensions
            x_g = self.gaussians(reconstructed_flat)
            x_g = torch.cat([10 * x_g[:, :self.num_of_gaussians] , x_g[:, self.num_of_gaussians:] ], dim=1)            
