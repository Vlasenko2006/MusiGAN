...
class ShallowVariationalEncoderDecoder(nn.Module):
    def __init__(self, input_dim, latent_dim=128):
        super(ShallowVariationalEncoderDecoder, self).__init__()

        # Encoder: Single Conv1D layer for shallow encoding
        ...
        self.encoder_bn = nn.BatchNorm1d(latent_dim)  # Batch Normalization for encoder

        # Fully connected layers for latent space
        self.fc_mu = nn.Linear(latent_dim, latent_dim)
        self.fc_logvar = nn.Linear(latent_dim, latent_dim)

        # Decoder: Single ConvTranspose1D layer for shallow decoding
        self.latent_to_decoder = nn.Linear(latent_dim, latent_dim)
    ...
    def encoder(self, x):
        """
        Encoder: Takes input and encodes it into a shallow latent representation.
        ...
        Returns:
            Tuple[torch.Tensor, torch.Tensor]:
                - mu: Mean of the latent space. [batch_size, latent_dim, seq_len//2]
                - logvar: Log-variance of the latent space. [batch_size, latent_dim, seq_len//2]
        """
        x = self.encoder_conv(x)  # [batch, latent_dim, seq_len//2]
        x = self.encoder_bn(x)

        # Treat the sequence dimension as sequence for latent space
        x = x.permute(0, 2, 1)  # [batch, seq_len//2, latent_dim]

        mu = self.fc_mu(x)      # [batch, seq_len//2, latent_dim]
        logvar = self.fc_logvar(x)  # [batch, seq_len//2, latent_dim]
    ...
    def decoder(self, z):
        """
        Decoder: Takes sampled latent vectors and reconstructs the input.

        Args:
            z (torch.Tensor): Latent vector. [batch, seq_len//2, latent_dim]

        Returns:
            torch.Tensor: Reconstructed input. [batch, input_dim, seq_len]
        """
        x = self.latent_to_decoder(z)  # [batch, seq_len//2, latent_dim]
        x = x.permute(0, 2, 1)  # [batch, latent_dim, seq_len//2]
